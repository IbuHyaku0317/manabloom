...
function downloadBackup(backup: ManaBloomBackup, prefix = "manabloom-backup") {
  const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `${prefix}-${backup.exportedAt.slice(0, 19).replace(/[:T]/g, "-")}.json`;
  link.click();
...
function Settings({ onRestored }: { onRestored: () => Promise<void> }) {
  ...
  async function exportData() { setBusy(true); try { downloadBackup(await createBackup()); } finally { setBusy(false); } }
  ...
  async function applyBackup() {
    ...
    setBusy(true);
    try {
      downloadBackup(await createBackup(), "manabloom-before-restore");
