...
function Home({ questions, attempts, sessions, onStudy, onCreate, onList }: {
  ...
  return <section className="page home-page">
    ...
    <aside className="reward-card" aria-label="楽天アフィリエイト広告">
      <div className="reward-copy"><small>広告・PR</small><h2>がんばった自分に、ご褒美はいかがですか？</h2><p>こちらのリンクを経由して購入いただくと、開発者に報酬が入り、ManaBloomを育てる励みになります。</p><p className="reward-online">※ オンラインのときにタップしてください。</p></div>
      {/* eslint-disable-next-line react/jsx-no-target-blank */}
      <a href="https://hb.afl.rakuten.co.jp/hsc/568d73c9.aab63da7.564bf542.832cabdd/?link_type=text&ut=eyJwYWdlIjoic2hvcCIsInR5cGUiOiJ0ZXh0IiwiY29sIjoxLCJjYXQiOjEsImJhbiI6Im5hbWUiLCJhbXAiOmZhbHNlfQ%3D%3D" target="_blank" rel="nofollow sponsored noopener" style={{ wordWrap: "break-word" }}>楽天市場</a>
    </aside>
  </section>;
}
...
function NavButton({ active, icon, label, onClick }: { active: boolean; icon: string; label: string; onClick: () => void }) { return <button type="button" className={active ? "active" : ""} onClick={onClick}><span>{icon}</span><small>{label}</small></button>; }
